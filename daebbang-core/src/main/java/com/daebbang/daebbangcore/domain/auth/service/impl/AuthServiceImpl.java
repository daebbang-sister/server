...
public class AuthServiceImpl implements AuthService {
    ...
    public TokenInfo rotateRefreshToken(String refreshToken) {
        ...
        String access = jwtUtils.createAccessToken(role, userId);
        String refresh = jwtUtils.createRefreshToken(role, userId);

        // 기존 redis에 있던 refresh 삭제 및 새로운 refresh 등록
        redisService.setData("RT:" + userId, refresh, Duration.ofMillis(jwtUtils.getRefreshTokenExpirationTime()));
    ...
    public TokenInfo issueTokenAndSaveToken(String username, String role) {
        Long userId = userService.getUser(username)
                                    .getId();
